...
import feedparser import openai import json import config import os import requests import html from bs4 import BeautifulSoup from jinja2 import Environment, FileSystemLoader from dateutil import parser as dateparser from datetime import datetime, timezone import re
...
with open(os.path.join(BASE_DIR, "feeds.json"), encoding="utf-8") as f: feeds = json.load(f)

Load prompts

def load_prompt(path): with open(os.path.join(BASE_DIR, path), "r", encoding="utf-8") as f: return f.read()

clustering_prompt_template = load_prompt("prompts/clustering_prompt.txt") synthesis_prompt_template = load_prompt("prompts/synthesis_prompt.txt")

Load rss_template.xml first line

with open(os.path.join(BASE_DIR, "templates/rss_template.xml"), encoding="utf-8") as f: rss_template_version = f.readline().strip()
...
def clean_xml_headers(text): return re.sub(r'\s*<?xml[^>]+??>\s*', '', text, flags=re.IGNORECASE)

def strip_rss_tags(text): text = re.sub(r'</?rss[^>]>', '', text, flags=re.IGNORECASE) text = re.sub(r'</?channel[^>]>', '', text, flags=re.IGNORECASE) return text.strip()

def fetch_og_image(url): try: response = requests.get(url, timeout=10) if response.status_code == 200: soup = BeautifulSoup(response.text, "html.parser") og_image = soup.find("meta", property="og:image") if og_image and og_image.get("content"): return og_image["content"] first_img = soup.find("img") if first_img and first_img.get("src"): return first_img.get("src") except Exception as e: print(f"⚠️ Error fetching image from {url}: {e}") return None

def filter_recent_articles(articles, now): recent = [] for entry in articles: pub_date = entry.get("pubDate") if isinstance(pub_date, str): try: pub_date = dateparser.parse(pub_date) except Exception: pub_date = now if pub_date.tzinfo is None: pub_date = pub_date.replace(tzinfo=timezone.utc) age_hours = (now - pub_date).total_seconds() / 3600 if age_hours <= config.RUN_INTERVAL_HOURS: recent.append(entry) return recent

def enforce_max_articles(articles): return articles[: config.MAX_ARTICLES]

def generate_synthesis(json_articles, max_attempts=3): prompt_body = synthesis_prompt_template + "\n\n" + json.dumps(json_articles, indent=2) attempts = 1 while attempts <= max_attempts: response = openai.chat.completions.create( model="gpt-4o", messages=[{"role": "user", "content": prompt_body}], max_tokens=4000 ) output = response.choices[0].message.content.strip() output = clean_xml_headers(output) output = strip_rss_tags(output) if len(output) >= MIN_CHARACTERS or attempts == max_attempts: return output extra = ( f"\n\nPlease expand the above synthesis to reach at least {MIN_CHARACTERS} characters, " "maintaining a calm and mindful tone." ) prompt_body += extra attempts += 1 return output

def main(): now = datetime.now(timezone.utc)
